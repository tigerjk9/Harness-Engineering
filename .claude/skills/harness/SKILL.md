...
교육 앱이라면 → `[edu-harness 스킬 사용]`을 사용하세요.
...
[harness 스킬 사용] 파일 업로드 API를 만들어줘.
[harness 스킬 사용] 대시보드 페이지를 만들어줘. WCAG 2.1 AA 준수.
```
...
```
[execution-loop 스킬 사용] verify 실패 항목을 수정해줘.
APPROVED 또는 CONDITIONAL이 될 때까지 반복해줘.
```

수치 목표가 있다면:
```
[objective-loop 스킬 사용] verify SCORE를 현재 [N] → 80 이상으로 올려줘.
```
...
기능 완료 후 반드시 실행. **이 단계가 하네스를 진화시킵니다.**

```
□ 반복적으로 나온 문제    → CLAUDE.md 금지 행동에 추가
□ 효과적이었던 패턴       → CLAUDE.md 규칙에 추가
□ 새 아키텍처 결정        → architecture.md에 기록
□ verify.sh에 추가할 항목 → 발견 시 제안
```
...
단일 컴포넌트 / 단일 API  → [harness 스킬 사용] 직접 호출
3개 이상 파일 연동 기능   → Planner 승인 필수
수치 목표 달성 필요       → [objective-loop 스킬 사용] 병행
```
...
| 스킬 | 단독 호출 |
|------|-----------|
| execution-loop | `[execution-loop 스킬 사용] ...` |
| verify | `bash .claude/skills/verify/verify.sh` |
| objective-loop | `[objective-loop 스킬 사용] ...` |
